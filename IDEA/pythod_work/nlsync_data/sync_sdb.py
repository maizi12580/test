...
class SCSDB:
    ...
    def set_session_attributes(self,attri_options):
        self.db.set_session_attri(attri_options)


    def get_session_attributes(self):
        return self.db.get_session_attri()
                ...
                for sub_tbl in sub_tbls:
                    ...
                    for index_info in index_infos:
                        ...
                        try:
                            ...
                            print("error code: %s, error: %s" % (e.code, e))
        except (SDBTypeError, SDBBaseError), e:
            if self.log is None:
                print "failed to drop index for sub cl: %s.%s .error code: %s, error: %s" % (main_cs_name, main_cl_name, e.code, e)
            else:
                self.log.warn("failed to drop index for sub cl: %s.%s .error code: %s, error: %s" % (main_cs_name, main_cl_name, e.code, e))
    ...
    def sync_hisub_crtidx(self, main_cs_name, main_cl_name, indexes_objstr, date_fm, etl_date='', dropidx_num=2):
        try:
            ...
            if self.log is None:
                print "failed to create index for sub cl: %s.%s .error code: %s, error: %s" % (main_cs_name, main_cl_name, e.code, e)
            else:
                self.log.warn("failed to create index for sub cl: %s.%s .error code: %s, error: %s" % (main_cs_name, main_cl_name, e.code, e))
...
def main():
    ...
    cl_name = 'bpcofhsh'
    #创建表
    connect_to_hosts= [{'host':'T-DSJ-HISDB01','service':'31810'},{'host':'T-DSJ-HISDB03','service':'31810'},{'host':'T-DSJ-HISDB03','service':'31810'}]
    db = SCSDB(hostname, svcport, username, password,connect_to_hosts)
    # print db.get_session_attributes()
    # for i in range(1000):
    #     db.set_session_attributes({"PreferedInstance":"M"})
    #     master_count = db.sync_get_count(cs_name,cl_name) 
    #     print "master_count:%s" % master_count
    #print db.get_session_attributes() 
    #print db.get_session_attributes()
    db.set_session_attributes({"PreferedInstance":"S","PreferedInstanceMode":"ordered"})
    for i in range(1000):
        slave_count = db.sync_get_count(cs_name,cl_name) 
        print " slave_count:%s" % slave_count
