...
class SyncMetaData:
    ...
    def get_field_info(self, column_name, data_type_name, column_length, scale,remark,is_decimal):
        sdb_type = None
        if "CHARACTER" == data_type_name or "CHAR" == data_type_name:
            sdb_type = "string"
        elif "VARCHAR" == data_type_name:
            sdb_type = "string"
        elif "LONG VARCHAR" == data_type_name:
            sdb_type = "string"
        elif "INTEGER" == data_type_name or "INT" == data_type_name:
            sdb_type = "int"
        elif "SMALLINT" == data_type_name:
            sdb_type = "int"
        elif "BIGINT" == data_type_name:
            sdb_type = "long"
        elif "REAL" == data_type_name:
            sdb_type = "float"
        elif "DOUBLE" == data_type_name:
            sdb_type = "double"
        elif "TIMESTAMP" == data_type_name:
            sdb_type = "string"
        elif "DATE" == data_type_name:
            sdb_type = "string"
        elif "TIME" == data_type_name:
            sdb_type = "string"
        elif "DECIMAL" == data_type_name and "0" == scale.strip() and int(column_length) <= 8:
            #print "type: %s, cmp: %s, scale: %s"%(type(scale),"0"==scale,scale)
            #self.log.error("column_name: %s, data_type_name: %s, column_length: %s, scale: %s,to int"%(column_name, data_type_name, column_length, scale))
            sdb_type = "int"
        elif "DECIMAL" == data_type_name and "0" == scale.strip() and int(column_length) > 8:
            #self.log.error("column_name: %s, data_type_name: %s, column_length: %s, scale: %s,to long"%(column_name, data_type_name, column_length, scale))
            sdb_type = "long"
        elif "DECIMAL" == data_type_name and "0" != scale.strip():
            #self.log.error("column_name: %s, data_type_name: %s, column_length: %s, scale: %s,to double"%(column_name, data_type_name, int(column_length), scale))
            if is_decimal:
                sdb_type = "decimal"
            else:
                sdb_type = "double"
    ...
    def get_table_records(self, table_name,collectionSpace,rnum,collection=None):
        ...
        for schema in self.ods_schema:
            ...
            for db2_table_rd in db2_table_rds:
                ...
                if "DECIMAL" == db2_field[2] and "0" != db2_field[4].strip():
                    is_decimal = is_Decimal(tbl_format["tbl_name"],db2_field[1].lower())
    ...
    def is_Decimal(self,tbl_name,decimal_column):
        condition = {"tbl_name":tbl_name}
        tbl_in_his = self.db.sync_query(mdm_metahis_cs,mdm_metahis_cl,condition)
        if len(tbl_in_his) ==0:
            return True
        else:
            tbl_in_his[0]
