...
class SyncMetaData:
    def __init__(self):
        ...
        self.db2_describe_table = '%s/db2 "SELECT ' \
                                  ...
                                  't.LENGTH||\'|\'||' \
                                  't.SCALE||\'|\'||' \
                                  'CASE WHEN t.REMARKS is null THEN \'None\' END as field FROM SYSCAT.COLUMNS t ' \
    ...
    def get_field_info(self, column_name, data_type_name, column_length, scale,remark):
        ...
        field_info = "%s|%s|%s" % (column_name, sdb_type, None)

        return field_info

    def get_table_records(self, table_name,collectionSpace,rnum):
        # 1.get tables
        tables = []
