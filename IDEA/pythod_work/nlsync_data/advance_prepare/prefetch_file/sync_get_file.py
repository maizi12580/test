...
class SyncGetFile:
    ...
    # check remote file and get size of dat and ok
    def check_remote_file(self, get_mode, frm_host, frm_file, frm_user, frm_passwd, timeout=7200):
        try:
            s = pxssh.pxssh(timeout=timeout)
            s.login(frm_host, frm_user, frm_passwd, original_prompt='[$#>]')
            ret_arr = []
            #1.check remote file
            frmok_file = frm_file.split('.')[0] + '.ok'
            tmp_command = 'pwd'
            chk_frmfile = 'ls -al %s' % frm_file
            chk_frmok = 'ls -al %s' % frmok_file
            s.sendline(chk_frmfile)
            s.prompt()
            ret1 = s.before
            file_size = ret1.split('\n')[1].split(' ')[4]
            s.sendline(chk_frmok)
            s.prompt()
            ret2 = s.before
            ok_size = ret2.split('\n')[1].split(' ')[4]
            ret_arr.append(file_size)
            ret_arr.append(ok_size)
            print 'check get file ret: %s' % (ret_arr)
            self.log.info('check get file ret: %s' % (ret_arr))
            return ret_arr
            s.logout()
            #
        except pxssh.ExceptionPxssh, e:
            print 'pxssh failed on login. error: %s' % str(e)
            self.log.warn('pxssh failed on login. error: %s' % str(e))
    ...
    def check_remote_file(self, frm_host, frm_file, frm_user,
        ...
        if self.log is None:
            ...
            self.log.info("check ok file return: %s" % check_ok_ret)
        ...
        if self.log is None:
            ...
            self.log.info("check data file return: %s" % check_data_ret)

        if ok_ret[0].find(self.no_such_file) != -1 and \
           data_ret[0].find(self.no_such_file) != -1:
    ...
    def get_file(self, frm_host, frm_file, frm_user, sync_file,
        ...
        while not file_ready:
            ...
            file_ready = self.check_local_file(sync_file, check_file_size)
