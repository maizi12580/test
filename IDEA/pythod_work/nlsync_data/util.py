...
class SYNC:
    def __init__(self, tbl_name, hostname='localhost', svcport='11810',
        ...
        # sesstion attribute
        self.master_session_attri = {"PreferedInstance":"M"}
        self.slave_session_attri = {"PreferedInstance":"S"}

    # get new meta table structure
    def sync_initialize_metatbl(self, sync_date=None, delta_dt=0):
        ...
        if sync_date is None:
            ...
            sync_dt = sync_date
        # 根据同步时间sync_date和间隔日期delta_dt确认获取元数据的时间
        if 0 != delta_dt:
            meta_date = (datetime.datetime.strptime(sync_dt, '%Y%m%d') +
                         abs(timedelta(days=int(delta_dt)))).strftime('%Y%m%d')
        else:
            meta_date = sync_dt
        update_meta_ok = sync_update_meta.update_meta_table(meta_date)
        if update_meta_ok:
    ...
    def file_parse_transcd(self, transcode_path):
        ...
        print "UTIL, TRANSCODE FILE: %s" % trans_file
    ...
    def data_import(self):
        ...
        # condition是对导入的数据量进行count时的条件，现在需要将"add_fields"中的字段名称也作为条件
        add_fields_list = sync_config.get_add_fields_list()
        # add_fields_list 返回的是使用","切割后的数组
        if add_fields_list is not None and len(add_fields_list) > 0:
            for add_field_item in add_fields_list:
                ...
                # 选择只有key和value的，比如时间字段带日期格式，不使用
                if len(field_info) == 2:
                    # 设置为condition
                    if field_info[1].find("%s") != -1:
                        condition[field_info[0].strip()] = field_info[1].strip() % sync_dt
                    else:
                        condition[field_info[0].strip()] = field_info[1].strip()
                # 如果长度为3(只能有一个)，表示是抽数日期字段，将其作为count条件，并且将原条件删除
                elif len(field_info) == 3:
                    del condition[etl_key]
                    etlvalue = datetime.datetime.strptime(sync_dt,'%Y%m%d').strftime(field_info[2].strip())
                    condition[field_info[0].strip()] = etlvalue
         
        # 对导入的数据量进行count时，使用的是import_csname和import_clname
        import_csname = sync_config.get_table_import_options_csname()
        import_clname = sync_config.get_table_import_options_clname()
        # 如果没有配置导入的集合与集合空间或者只配置了一个,使用下面的代码获取导入的集合与集合空间
        # if import_csname is None or import_clname is None:
        # 如果iprt_cmd中没有csname或者clname，就是使用原有逻辑获取import_csname和import_clname
        if self.full_type == sync_type:
            if import_csname is None or import_clname is None:
                ...
                self.log.info("TRUNCATE table, cs_name: %s,cl_name: %s " % (import_csname,import_clname))
            else:
                #  如果iprt_cmd中配置csname了clname(依赖表),在删除数据时，需要带条件
                ...
                self.log.info("REMOVE table, cs_name: %s,cl_name: %s ,REMOVING condition: %s; REMOVE Number: %s" % (import_csname,import_clname,condition,count))
            ...
            # 如果在集合中刚好已经存在一份数据count == file_name，在导入后进行判断时会失败，再次执行导入操作才删除已导入的数据
            ...
            count = self.scdb.sync_get_count(import_csname, import_clname, condition)
            # if table have today sync data, we need remove first
            # 如果数据没有被删除，那么会在import判断时失败，重新导入时会进入判断删除数据
        ...
        if self.full_type == sync_type and dest_table is not None and \
           sub_bound is not None:
            ...
            bef_count = self.scdb.sync_get_count(import_csname, import_clname, condition={}) 
        import_bftm = datetime.datetime.now()
        # format meta info
        meta_info = sync_meta.get_meta_ret()
        meta_fields = sync_import.formate_fields(meta_info, etl_key, date_fm, sync_dt,add_fields_list)
        ...
        if self.full_type == sync_type and dest_table is not None and \
           sub_bound is not None:
            ...
            aft_count = self.scdb.sync_get_count(import_csname, import_clname, condition={})
        ...
        if rec_file is not None:
            ...
            if int(file_num) == import_num:
                ...
                if 'datutf8' == iprfile_type:
                    ...
                    expect_file = import_file.split(".")[0] + ".except1"
                    self.log.info('remove import file is : %s' % import_file)
                    self.sync_os.remove_file(expect_file)
                    self.log.info('remove import expect file is : %s' % expect_file)
    ...
    def data_upsert(self, upsert_file):
        ...
        condition = dict()
        condition[etl_key] = sync_dtfm.strftime(date_fm)
        # condition是对upsert的数据量进行count时的条件，现在需要将"add_fields"中的字段名称也作为条件
        add_fields_list = sync_config.get_add_fields_list()
        # add_fields_list 返回的是使用","切割后的数组
        if add_fields_list is not None and len(add_fields_list) > 0:
            for add_field_item in add_fields_list:
        ...
        # 原来根据是否有依赖表判断，现在需要使用iprt_cmd判断
        if sync_config.get_rely_table_list() is not None or sync_config.get_table_import_options_map() is not None:
        ...
        self.log.info('upsert condition: %s, imported number: %s' % (condition,import_num))
        # udpate local table
        if flag != sync_flag[-1]:
            sync_flag.append(flag)
        else:
        ...
        self.log.info('upsert ret info: %s' % upsert_ret_info)
     
        if upsert_ret_info[0] == 0:
            is_upsert_ok = True
            if int(file_num) != import_num:
                print 'WARNNING,Table: %s, file number: %s, import number: %s' % (self.tbl_name, file_num, import_num)
            ...
            self.log.error("Upsert error,file_num is:%s,upsert_num is:%s" % (int(file_num),import_num))
        return is_upsert_ok
    ...
    def data_export(self):
        ...
        self.sync_os.remove_file(export_abs_file)
        # export
        export_cmd_options = sync_config.get_table_export_options_map()
        """
        为了防止主备未完全同步导致的sdbexport数据丢失，现在增加判断逻辑
        1. 获取当前的 sesstion_attribute，在判断完成后重新还原
        2. 设置当前的 sesstion_attribute，设置为从Master读取，并进行count
        3. 设置当前的 sesstion_attribute，设置为从Slave读取，并进行count
        4. 由于从Slave读取是随机读取其中一个Slave，所以循环查询5次，如果都相等则任认为同步完成
        5. 为了避免同步为完成而进入死循环，限定循环次数为15次，每次等待60秒，次数达到15次后，判定导出失败
        """
        current_session_attri = self.scdb.get_session_attributes() 
        self.scdb.set_session_attributes(self.master_session_attri)
        wait_sync_times = 0
        check_flag = True
        master_count = self.scdb.sync_get_count(self.cs_name,self.cl_name,condition=eval(matcher))
        """
        通过直连备节点并且删除几条数据，进行测试，结果如下
        1. 通过Master查询，不会感知到数据被删除，count值不变
        2. 通过Slave查询，会随机在两备节点中选择一个进行查询，count值有和Master相等和不等的两种情况
        3. Slave的从不同备节点读取的比例大概为：3:7 - 4:6，不是轮循或者1/2
        4. 设置session_attribute需要每次都设置再查询，否则会一直通过第一次查询的节点进行查询
        """
        time.sleep(60)
        while True:
            for i in range(10):
                self.scdb.set_session_attributes(self.slave_session_attri)
                slave_count = self.scdb.sync_get_count(self.cs_name,self.cl_name,condition=eval(matcher))
                if master_count != slave_count:
                    check_flag = False
                    self.log.warn("Matcher : %s , Master count:%s, Slave count:%s, wait 60 secondes for sync complete! Wait times: %d" % (eval(matcher),master_count,slave_count,wait_sync_times))
                    break
            if check_flag is True or wait_sync_times >= 15:
                break
            else:
                time.sleep(60)
                wait_sync_times += 1
        self.scdb.set_session_attributes(current_session_attri)
        if check_flag is not True:
            self.log.warn("Master Node to Slave Node synchronous is not complete!!!")
        ...
        if export_cmd_options is None:
            ...
            if "filter" in export_cmd_options and isinstance(export_cmd_options.get('filter'),basestring):
                filter = eval(export_cmd_options.get('filter'))
                if eval(matcher).keys()[0] in filter:
                    filter.update(eval(matcher))
                    matcher = str(filter)
                else:
                    matcher = export_cmd_options.get('filter')
        matcher = eval(matcher)
        export_num = self.scdb.sync_get_count(self.cs_name,self.cl_name,condition=matcher)
        export_aftm = datetime.datetime.now()
        export_tm = str(export_aftm - export_bftm)
        
        # get export file number
        real_export_num = self.sync_os.get_lineno(export_abs_file)
        if real_export_num == export_num:
            is_export_ok = True
            # touch ok file
    ...
    def update_sync_status(self, status, condition=None):
        tbl_cond = {"tbl_name": self.tbl_name}
        tbl_ret = self.scdb.sync_query(self.local_cs, self.local_cl, tbl_cond)
        begin_tm = tbl_ret[0].get("begin_tm")
        begin_fmt = datetime.datetime.strptime(begin_tm, '%Y-%m-%d %H:%M:%S.%f')
        end_time = datetime.datetime.now()
        #end_fmt = datetime.datetime.strptime(end_time, '%Y-%m-%d %H:%M:%S.%f')
        total_tm = long((end_time - begin_fmt).total_seconds())
        ruler = {"$set": {"sync_st": status, "end_tm": end_time, "total_tm": total_tm}}
    ...
    def clear_file(self):
        ...
        sync_date = sync_local.get_sync_date()
        # clear file
        sync_clear = SyncClear(sync_file, sync_date,self.log)
        ...
        self.log.info('[TABLE: %s][CLEAR FILE][BEFORE INFO: %s]' % (self.tbl_name, ruler))
        self.scdb.sync_update(sync_local_cs, sync_local_cl, ruler, matcher)
        self.log.info('[TABLE: %s][CLEAR FILE][AFTER INFO: %s]' % (self.tbl_name, ruler))
