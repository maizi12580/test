...
class SparkDDLSync:
    ...
    def exe(self):
        self.meta_cursor.execute(
            'select \"TBL_ID\",\"TBL_NAME\" from \"TBLS\" where \"DB_ID\"=(select \"DB_ID\" from \"DBS\" where \"NAME\"=\'{aim_db_name}\');'.format(
                aim_db_name=self.aim_db_name))
        meta_rows = self.meta_cursor.fetchall()
        try:
            for meta_row in meta_rows:
                self.meta_cursor.execute(
                    'select \"PARAM_VALUE\" from \"TABLE_PARAMS\" where \"TBL_ID\"={tbl_id} and \"PARAM_KEY\" like  \'spark.sql.sources.schema.part.%\' order by \"PARAM_KEY\"'.format(
                        tbl_id=meta_row[0]))
                ...
                # table name
                self.meta_cursor.execute(
                    'select \"PARAM_VALUE\" from \"SERDE_PARAMS\" where \"SERDE_ID\"={tbl_id} and \"PARAM_KEY\"=\'collection\''.format(
                        tbl_id=meta_row[0]))
                tb_name = self.meta_cursor.fetchone()
                if tb_name is None:
                    self.logger(self.level["error"], "table not found,tbl_id is {tbl_id}".format(tbl_id=meta_row[0]))
                    continue;
                full_ddl = 'create table  if not exists {aim_db_name}.{table} ({sub_ddl}) ;'.format(
                    aim_db_name=self.aim_db_name, table=meta_row[1], sub_ddl=sub_ddl, tbname=tb_name[0])
                full_ddl = full_ddl.replace("\"", "`")
                os.system(
                    "mysql -u {user} -p'{password}' -h '127.0.0.1' -e '{ddl}'".format(ddl=full_ddl, user=self.mysql_user,
