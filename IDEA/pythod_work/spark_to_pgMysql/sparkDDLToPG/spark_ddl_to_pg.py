...
class SparkDDLSync:
	...
	self.aim_db = self.config.get('psql','aim_db')
	...
	self.temp_conn = psycopg2.connect(database=self.aim_db, user=self.psql_user,\
                password=CryptoUtil.decrypt(self.psql_password), host=self.host, port=self.port)
    ...
    def exe(self):
	self.meta_cursor.execute('select \"TBL_ID\",\"TBL_NAME\" from \"TBLS\" where \"DB_ID\"=(select \"DB_ID\" from \"DBS\" where \"NAME\"=\'{aim_db}\');'.format(aim_db=self.aim_db))
